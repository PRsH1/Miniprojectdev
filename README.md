...
- **대상 서버 라우팅:**
  ...
  - **SP 시작** (`sso-login` → `auth`): 우선순위 ① 명시적 `target`(test/test2/dev, `sso-login?target=dev`) → ② SAMLRequest의 `AssertionConsumerServiceURL`(`extractAcsUrl` + `resolveTargetByAcsUrl`, 화이트리스트 검증) → ③ test 폴백
