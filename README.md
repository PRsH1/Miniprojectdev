...
- **Open API Tester** — Postman 스타일의 eformsign Open API 테스트 도구 (Beta), 요청 저장·히스토리 복원, 에러 코드 모음 페이지 연동
...
| 엔드포인트 | 메서드 | 설명 |
|-----------|--------|------|
| `GET /memberV2` | GET | 멤버/그룹 관리 V2 (쿠키 인증 보호, 현행) |
| `GET /OpenAPIAutoTest` | GET | OPA 자동 테스트 페이지 (쿠키 인증 보호) |
| `GET /templatecopy` | GET | 템플릿 복제 도구 (쿠키 인증 보호) |
| `GET /idptestauth` | GET | IdP 테스트 페이지 (쿠키 인증 보호) |
| `GET /api/member` | GET | (deprecated) 구 멤버 관리 페이지 |
...
`API(JS,HTML)/OpenAPITesterProd.html` (공개 배포용) · `API(JS,HTML)/OpenAPITester.html` (전체 포함) + `assets/js/openapi/` (분할 모듈)
