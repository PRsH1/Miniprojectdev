...
- **알림 시스템** — 회원가입 요청·버그 리포트 등 주요 이벤트를 admin 상단 바 벨 아이콘으로 실시간 알림. DB 기반 모듈형 구조로 신규 알림 타입 확장 용이
- **커뮤니티** — 개발자 노트(마크다운 지원, admin 전용 작성), 버그 리포트 게시판(로그인 사용자 제보 → admin 알림)
...
│   ├── notifications.js          # ★ 알림 조회 + 읽음 처리 API (admin 전용)
...
├── assets/js/
│   ├── auth-status.js            # ★ 전 페이지 공통 로그인 상태 상단 바 (IIFE, admin 알림 벨 내장)
...
| `bug_reports` | `id, title, description, reporter_user_id(UUID FK), severity(low/normal/high/critical), status(open/in-progress/resolved/closed), admin_note, created_at, updated_at` |
...
| POST | `/api/bug-reports/:id/update` | admin | 상태/메모 수정 (PATCH 우회용 — Admin Console에서 사용) |
...
- **백엔드**: `controllers/notifications.js` — admin 전용 조회/읽음 처리 API
...
| 파일 | 설명 |
|---|---|
| `assets/js/auth-status.js` | 기존 상단 바에 알림 벨 통합. admin + 기본 모드에서만 활성화 (CORNER 모드 제외) |
...
- **폴링**: 60초 interval로 `/api/notifications` 호출 → 미읽음 배지 자동 갱신
