...
│   │
│   │   # ── SAML ────────────────────────────────────────────
│   ├── auth.js                 # SAML 응답 생성 (SP개시: 요청 ACS/target으로 test·dev 대상 라우팅, createTemplateCallback로 email/name 속성 채움, SAML_DEBUG 게이트 로깅)
│   ├── sso-login.js            # SAML SSO 로그인 폼 (target 쿼리로 대상 서버 강제 가능, debug=1 게이트 플래그 폼 전달)
...
│
├── lib/
│   └── saml.js                 # SAML IdP/SP 설정 (samlify, ACS_URLS test/dev 대상 서버별 SP + resolveTarget, loginResponseTemplate.context + createTemplateCallback로 AttributeStatement 생성, debugDeliver 진단 헬퍼)
│
├── auth/
