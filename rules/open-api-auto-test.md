...
### 응답 판정 (PASS / CHECK / FAIL / SKIP)

`request()`가 응답 status와 body를 보고 step 결과를 4단계로 판정한다.

| 상태 | 조건 | 색/배지 |
|---|---|---|
| `PASS` | status가 `ok` 배열에 포함 (+ `validate` 통과) | 초록 |
| `CHECK` | status가 `check` 배열(`CHECK_STATUS_CODES = [400, 401, 402, 403]`)에 포함 | 주황 (검토 필요) |
| `FAIL` | 그 외 status(404 포함) **또는** 200이지만 `validate`가 실패 사유 반환 | 빨강 |
| `SKIP` | 선행 step FAIL로 미실행 / `CACHE` = seed 캐시 재사용 | 노랑 |

- **CHECK(검토 필요)**: 정리·삭제 step(`createMember`, `deleteMember`, `deleteGroup`, `cleanupBulk1/2`, `cancelDocs`, `deleteDocs`)은 `ok:[200]` + `check: CHECK_STATUS_CODES`. 의도된 400~403(이미 정리된 리소스 등)인지 실제 오류인지 사용자가 결과 보기로 확인. CHECK는 `opaFailed`를 세팅하지 않아 파이프라인이 계속된다.
- **404는 CHECK 아님 → FAIL**: eformsign API 정리 step은 4xx로 400~403만 반환하므로 404는 실제 오류로 본다.
- **`validate(json)` 훅**: 200 OK여도 body가 실패를 나타내면(예: 일괄 처리 `success:false`) FAIL로 전환. 반환값은 `null`(정상) 또는 실패 사유 문자열. 응답 body 원문은 유지되고 `[검증 실패] {사유}` 가 앞에 덧붙는다 — 어느 항목이 실패했는지 그대로 확인 가능. 현재 `bulkCreateMembers`(OPA 030)에 적용. 다른 배치 API도 `validate`만 추가하면 동일 확장.
- **구 방식 주의**: OPA 030은 과거 `after`-throw로 검증했으나 응답 body가 에러 메시지로 덮여 사라지는 문제가 있어 `validate`로 교체됨. 신규 body 검증은 `after`-throw가 아닌 `validate`를 사용할 것.
...
- OPA 판정 우선순위: **FAIL > CHECK > SKIP > PASS** — 하나라도 FAIL → FAIL / FAIL 없고 CHECK 있음 → CHECK / 전부 SKIP → SKIP / 그 외(PASS·CACHE) → PASS
- 리포트에 "실패 상세"와 별도로 **"검토 필요 상세(CHECK)"** 섹션 포함 (모달·Markdown·HTML 모두)
...
- **Step별 요청/응답 인라인 펼침**: OPA별 결과 표의 각 step 행 아래에 표준 `<details>` 행이 추가되어 Request Body·Response를 펼쳐 볼 수 있다. 모달·내보낸 HTML(JS 없이 동작)·Markdown(GitHub 호환 `<details>`) 모두 반영. 본문은 `truncateBody()`로 절단(기본 16384자)하며, 본문 없는 step은 "(본문 없음)" 표기. 공통 헬퍼: `truncateBody()` / `stepDetailToRow()`

#### 프로필별 실행 이력 복원

- 사이드바 "프로필별 실행 이력" 항목을 클릭하면 우측 결과 패널에 과거 실행 결과 행이 복원되고(`loadHistoryRun(id)`), "리포트 보기"로 해당 실행 리포트를 다시 열 수 있다. 복원 시 결과 패널 상단에 `#historyViewBanner` 경고 배너(과거 이력 안내)가 표시되고, 새 실행을 시작하면 배너가 사라지며 라이브로 복귀한다.
- 이력은 localStorage(`open-api-auto-test-history-v1`)에 프로필명 키로 최대 10건 저장되며, 각 엔트리에 `id` + 리포트 스냅샷(`report`)이 포함된다. 저장 시 `snapshotReportForHistory()`가 본문을 `truncateBody`로 절단해 용량을 절감하고, `QuotaExceededError` 발생 시 가장 오래된 엔트리부터 제거하며 재시도(최소 1건) → 그래도 실패하면 `report`를 뺀 요약-only 엔트리로 저장한다(`persistHistoryMap()`). `report`가 없는 구(舊) 엔트리는 클릭 비활성("상세 데이터 없음" 표기).
