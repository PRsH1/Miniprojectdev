...
현재 프로젝트는 요청 유형에 따라 에러 응답을 분기한다.

## 정책

- `/api/*` 요청은 항상 JSON 에러를 반환한다.
- `/app/*` 요청은 HTML 에러 페이지를 반환한다.
- 인증이 필요한 페이지에서 인증이 없는 경우에는 기존 로그인 리다이렉트 흐름을 유지한다.
- 내부 예외 상세, 스택, DB 세부 정보, 시크릿은 응답 본문에 노출하지 않는다.
- 내부 상세는 서버 로그로 남기고, 사용자에게는 이해 가능한 원인과 다음 액션만 전달한다.
...
페이지 요청에서 위 상태 코드가 발생하면 공통 유틸이 해당 정적 파일을 읽어 반환한다. 파일 읽기에 실패해도 최소 HTML fallback 응답을 반환한다.
...
- 중앙 라우터 적용: `api/index.js`
- 페이지 인증/권한 처리 적용: `controllers/_shared/auth-middleware.js`

## 검증 기준

- 없는 `/api/...` 경로는 JSON 404
- 없는 `/app/...` 경로는 HTML 404
- API 잘못된 메서드는 JSON 405
- 서버 내부 실패 시 API는 JSON 500/502, 페이지는 HTML 500
