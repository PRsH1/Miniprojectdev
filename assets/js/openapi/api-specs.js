...
const API_SPECS = {
    ...
    'OPA2_005': {
        ...
        requestBody: [
            ...
            { key: 'document.recipients[].step_type', type: 'string', required: false, description: '워크플로우 단계 타입', note: '"01"(완료), "02"(결재), "03"(외부 수신자), "04"(내부 수신자), "05"(참여자), "06"(검토자) — recipients 내 각 객체에 필수 설정' },
            { key: 'document.recipients[].id', type: 'string', required: false, description: '단계 처리자 ID (구형 워크플로우)', note: '1) step_type이 결재일 경우 필수. 2) step_type이 내부 수신자이고 그룹/멤버 선택 전송 옵션이 활성화된 경우 필수. 그룹으로 설정하려면 그룹 이름 또는 ID 입력' },
            { key: 'document.recipients[].name', type: 'string', required: false, description: '단계 처리자 이름 (구형 워크플로우)', note: 'step_type이 외부 수신자일 경우 필수' },
            { key: 'document.recipients[].email', type: 'string', required: false, description: '단계 처리자 이메일 (구형 워크플로우)', note: 'step_type이 외부 수신자일 경우, 템플릿의 알림 채널 설정에 따라 필수' },
            { key: 'document.recipients[].sms', type: 'object', required: false, description: '단계 처리자 휴대폰 정보 (구형 워크플로우)' },
            { key: 'document.recipients[].sms.country_code', type: 'string', required: false, description: '단계 처리자 휴대폰 국가 코드 (구형 워크플로우)' },
            { key: 'document.recipients[].sms.phone_number', type: 'string', required: false, description: '단계 처리자 휴대폰 번호 (구형 워크플로우)' },
            { key: 'document.recipients[].auth', type: 'object', required: false, description: '인증 및 전송 기한 정보' },
            { key: 'document.recipients[].auth.password', type: 'string', required: false, description: '인증 비밀번호', note: '신형: 워크플로우 설정에서 본인확인 정보 체크 시 입력. 구형: step_type이 외부 수신자이고 비밀번호 설정 체크 시 입력' },
